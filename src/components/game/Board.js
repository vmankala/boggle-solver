...
class Board extends Component {
    constructor() {
        super();
        this.state = {
            selected: 0, // currently highlighted cell
            tints: ['#e4e7eb', 'tint1', 'tint1', 'tint1', 'tint1', 'tint1', 'tint1', 'tint1', 'tint1', 'tint1', 'tint1', 'tint1', 'tint1', 'tint1', 'tint1', 'tint1'], // cosmetic tint (applied to selected)
            chars: ['', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '']
        }
    ...
    select(id) { // switches selected to id, modifies tint accordingly
        if (id === -1) { // -1 represents nothing selected
    ...
    keyInput(e) {
        if (e.keyCode >= 65 && e.keyCode <= 90 && this.state.selected !== -1) { // letter keys
            let newchars = this.state.chars;
            newchars[this.state.selected] = (e.key).toLowerCase();
            this.setState({
                chars: newchars,
            });
            if (this.state.selected !== 15) { // automatically move selected forward
    ...
    componentDidUpdate() {
        if (this.props.clearing) { // check if the game needs the board to be cleared
            this.props.clear(); // pass the clear callback up to parent to confirm clearing
            this.setState({
                chars: ['', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '']
            });
            this.select(0);
